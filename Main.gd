...
onready var instrumentTab= $VBoxContainer/EditorView/Instruments
...
func loadInstruments():
	...
	while instrument_name != "":
		if dir.file_exists("./%s/instrument.gd" % instrument_name):
			...
			instrumentTab.add_item(instrument_name, load("%s/%s/icon.png" % [dir.get_current_dir(), instrument_name]))
		else:
