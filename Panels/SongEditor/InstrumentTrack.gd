...
#onready var disabled = Global.piano_roll.piano.get_node("Disabled/ColorRect")
...
#	window_size_changed(OS.window_size)

func set_instrument(icon: Texture, instrument_name: String) -> void:
	$HBoxContainer/InstrumentToggle.icon = icon
	$HBoxContainer/InstrumentToggle.text = instrument_name
...
#	var _n = new_segment.connect("gui_input", self, "segment_input", [new_segment])
#	_n = new_segment.connect("button_down", new_segment, "set_disabled", [true])
