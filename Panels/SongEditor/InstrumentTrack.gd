...
onready var scroll_view = $HBoxContainer/ScrollView
...
func add_segments():
	...
	for i in range(scroll_view.rect_size.x/19):
	...
	if event is InputEventMouseButton:
		if event.pressed:
			if event.button_index == 1:
				...
				if get_index() > Global.last_segment:
					Global.last_segment = get_index()
